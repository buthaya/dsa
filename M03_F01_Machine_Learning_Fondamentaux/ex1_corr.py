...
--> L'arbre peu profond (max_depth=4) est souvent un bon compromis entre biais et variance.
2. L'arbre profond (max_depth=None) est-il meilleur ou pire que l'arbre 
   contraint ? Que vous dit cela sur l'overfitting ?
--> L'arbre profond est souvent pire que l'arbre contraint, ce qui indique un fort overfitting sur les données d'entraînement.
3. Le GLM donne des coefficients interprétables. Que dit le coefficient 
   du BonusMalus ? Est-ce cohérent avec votre intuition ?
--> Un coefficient négatif pour BonusMalus indiquerait que plus le bonus est élevé (meilleur conducteur), moins la fréquence de sinistre est élevée, ce qui est cohérent avec l'intuition.
...
--> Ici c'est un inconvénient, car la loi de Poisson est un modèle naturel pour la fréquence de sinistre. 
5. En pratique, pourquoi un actuaire utiliserait-il les deux approches 
   de manière complémentaire ?
--> Le GLM est utile pour l'interprétation et la compréhension des facteurs de risque, tandis que l'arbre de décision peut capturer des interactions complexes et des non-linéarités que le GLM ne peut pas modéliser.
   """